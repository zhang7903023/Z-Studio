...
function parsePrice(value) {
  ...
  return Number.isFinite(numeric) ? { priceCny: numeric, priceText: text } : { priceCny: null, priceText: text };
}
...
export async function buildSourceCatalog() {
  const [csv, mappingJson] = await Promise.all([fs.readFile(catalogCsvPath, "utf8"), fs.readFile(mappingPath, "utf8")]);
